...
public class OrderMicroserviceApplication {
	...
	public static void main(String[] args) {
		log.info("Inside OrderMicroservice application");
	...
	public ResponseEntity<ArrayList<OrderItemResponse>> getOrderItems(){
		 ...
		 * we get list of orderItems available from orderItem micro service and from among them, we can choose two items 
	...
	public ResponseEntity<Order> placeOrder(@RequestBody @Valid Order order,Locale locale) {
		...
		if (orderCreated == null) {
			log.error("Something went wrong order was not placed!!");
			throw new OrderNotCreatedException(message.getMessage(ErrorCodes.ORDERNOTCREATED, null, locale));
		}else {
			log.debug("Order placed successfully {}",orderCreated);
